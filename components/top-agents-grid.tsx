...
export function TopAgentsGrid({ initialMembers, walletAddress }: TopAgentsGridProps) {
  ...
  const filteredMembers =
    selectedCategory === "all" ? members : members.filter((member) => member.category === selectedCategory)
      ...
      <div className="flex flex-wrap gap-2">
        {categories.map((category) => (
          <Button
            ...
            className={
              selectedCategory === category ? "bg-blue-600 hover:bg-blue-700" : "border-gray-700 hover:border-blue-500"
            }
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {sortedMembers.map((member) => (
          <MemberCard
            ...
            member={member}
            onVote={handleVote}
            userVote={userVotes[member.id]}
            isVoting={votingStates[member.id]}
            showVoteButtons={!!walletAddress}
          />
        ))}
      </div>
